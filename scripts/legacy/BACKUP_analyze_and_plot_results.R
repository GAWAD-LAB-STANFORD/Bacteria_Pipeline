...
## uncomment lines 13-43 when actually want to use these inputs and run pipeline all the way through
#option_list = list(
...
## comment lines 46-53 if using the option inputs to run the full pipeline
#setwd("/oak/stanford/groups/cgawad/Infectious_Microbes_Studies/SC_Bacterial_PTA_Seq/2023_03_17_test")
# opt <- list(project = "scBac_PTA_Ecoli_sc_plate2well4A_S62", identify = "scBac_PTA_Ecoli_sc_plate2well4A_S62", 
#             sample_read_count_filename = "SC_Bacterial_PTA_Seq2.sample_read_counts.tsv",
#             summed_read_targets_filename = "scBac_PTA_Ecoli_sc_plate2well4A_S62_summed_read_targets.tsv",
#             contig_read_targets_filename = "scBac_PTA_Ecoli_sc_plate2well4A_S62_contig_read_targets.tsv",
#             contig_data_filename = "SC_Bacterial_PTA_Seq2.contig_data.tsv",
...
##this is a temprorary test I'm doing to debug something -Shawn
setwd("/oak/stanford/groups/cgawad/Infectious_Microbes_Studies/SC_Bacterial_PTA_Seq/20230405_scBac_JGI_Cultured_Envi_WGS")
 opt <- list(project = "scBac_PTA", identify = "scBac_PTA",
              sample_read_count_filename = "scBac_PTA.sample_read_counts.tsv",
                           summed_read_targets_filename = "scBac_WGAX_Pputida_sc_well3H_S41_summed_read_targets.tsv",
                                        contig_read_targets_filename = "scBac_WGAX_Pputida_sc_well3H_S41_contig_read_targets.tsv",
                                                     contig_data_filename = "scBac_PTA.contig_data.tsv",
                                                                  kraken_db_types = "microbial-plasmid-viral", kraken_jtree_suffix = ".kraken_jtree.json",
                                                                               blast_db_types = "nt-plasmid-viral", blast_results_suffix = ".blast_results.tsv",
                                                                                            ncbi_annotations_dir = "0", contig_alignment_fraction_min = 0.9,
                                                                                                         add_genus=FALSE)
...
#I wonder if we can just remove teh string taxomonic variable and only use genus
...
if (! "read_count" %in% colnames(contig_read_targets_df)) {
  if ("...1" %in% names(contig_read_targets_df)) {
